...
import { COURIER_PARTNERS, type CourierOrderSummary } from '@/features/courier/data'
...
function CourierWorkspacePage() {
  ...
  const runCheck = async () => {
    ...
    try {
      ...
      if (heavy.status === 'fulfilled') {
        if (form.productType !== 'heavy') next.push({ id: 'delhivery-heavy', title: 'Delhivery Heavy', subtitle: 'Heavy shipment check', state: 'off', summary: 'Heavy API skipped because product type is not Heavy.', rows: ['Switch product type to Heavy to run this API.'] })
        else {
      ...
      } else next.push({ id: 'delhivery-heavy', title: 'Delhivery Heavy', subtitle: 'Heavy shipment check', state: 'error', summary: errMsg(heavy.reason, 'Delhivery Heavy check failed'), rows: ['Heavy serviceability request did not succeed.'] })
