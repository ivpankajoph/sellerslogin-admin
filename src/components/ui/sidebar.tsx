...
function Sidebar({
  ...
  const { isMobile, state, openMobile, setOpenMobile, setOpen, hoverLock } =
    useSidebar()
  ...
  return (
    <div
      ...
      data-slot='sidebar'
      onMouseEnter={() => {
        if (!isMobile && collapsible === 'icon' && !hoverLock) {
          setOpen(true)
        }
      }}
      onMouseLeave={() => {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        'md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ms-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow-sm md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ms-1',
        className
      )}
      {...props}
    />
